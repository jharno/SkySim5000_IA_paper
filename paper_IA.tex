...
\newcommand{\Cora}[1]{\textcolor{teal}{#1}}
...
\title[Lensing beyond 2pt: accounting for IA]{Intrinsic alignments from projected tidal fields: non-linear  impact on cosmic shear probes (Mention source clustering as well?)}
...
\input{sections/0_abstract_keywords}
...
\input{sections/2_cosmic_shear_stats}
\input{sections/3_ia_models}
\input{sections/4_simulations}
\input{sections/5_validation}
\input{sections/6_higher_order_wl_stats}
\input{sections/7_conclusion}
\input{sections/8_acknowledgements}
\input{sections/9_data_availability}
