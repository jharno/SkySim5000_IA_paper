...
\label{eq:C_ell}
\end{eqnarray}
where $\ell$ are angular multipoles, $k$ are amplitudes of Fourier modes, $\chi_{\rm H}$ is the comoving distance to the horizon and the lensing kernels $q^{i}$ and $q^j$ are given by:
\begin{eqnarray}
...
\label{eq:q_lensing}
\end{eqnarray}
In the previous expression, $c$ is the speed of light, $H_0$ the Hubble parameter, $n^i(z)$ refers to the redshift distribution  in tomographic bins `$i$', while $a(\chi)$ is the scale factor at comoving distance $\chi$ from the observer.
...
This work focuses only on auto-tomographic bins and bin pairs, yet our methods and results can be straightforwardly extended to include these higher-order redshift combinations.

%\Cora{[Mention Euclid HOWLS paper somewhere?]}
...
\begin{enumerate}
    ...
    \item {\it Third-order aperture mass}:  \JHD{Lucas/Laila, can you summarise the measurements and modelling, and provide references?}
    The third-order aperture mass statistics, $M_{\rm ap}^3$, describes the third moment of the convergence field, smoothed by a compensated filter function \citep{Schneider1998}. For our analysis we choose the exponential filter from \citep{Crittenden2002}. For the construction of the individual aperture mass maps the smoothing is performed on the curved sky with help of the {\sc smoothing} and {\sc beam2bl} functions of the {\sc healpy} package \citep{Zonca2019}. For estimating the multiscale and tomographic third-order aperture mass we perform a spatial average over the product of three aperture mass maps with smoothing radii $(\theta_i,\theta_j,\theta_k)$ for the tomographic redshift bin combination $(l,m,n)$. Due to the symmetry properties of $M_{\rm ap}^3$ we only consider the configurations for which $\theta_i \leq \theta_j \leq \theta_k$ and $l\leq m\leq n$. For the theoretical model we choose the description outlined in \citep{Heydenreichetal2023} which is based on the relation of $M_{\rm ap}^3$ to the matter bispectrum and the latter is being modelled by the {\sc BiHalofit} fitting function \citep{Takahashietal2020}.
   ...
   Peak count statistics are amongst the most widely used non-Gaussian lensing statistics, as it is simple to measure yet highly sensitive to non-Gaussian structures.
    ...
    \item \textit{Lensing PDF}: The lensing one-point function, also referred to as the $\kappa$-PDF, can be computed directly from the histogram of the pixel values of a lensing map, and can further be modelled by large deviation theory, as recently shown in  \citet{LensingPDF}. The modelling is accurate provided that the map is smoothed on scales of a few arcmin to erase the highly non-linear structures. We use a top-hat beam of  1 arcmin to achieve this, although this is not strictly required if one is willing to replace the analytical modelling by a simulation-based inference model, as shown in \citet{Giblin_PDF}. 
\end{enumerate}
